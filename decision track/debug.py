...
trace = pd.DataFrame(columns=['alg', 'state', 'action', 'reward'])
    ...
    for i in range(len(algs)):
        ...
        for ep in range(test_iter):
            ...
            for _ in range(min(n_steps, config.num_stream_jobs)):
                ...
                step += 1
                trace.loc[-1] = [alg, state, action, reward]
    ...
    for ep in range(iter):
        ...
        for _ in range(min(n_steps, config.num_stream_jobs)):
            ...
            if step > 0:
                ...
                rewards.append(reward)
            step += 1
            trace.loc[-1] = ["random", state, action, reward]
            trace.index = trace.index + 1
            trace = trace.sort_index()
    ...
    for ep in range(iter):
        ...
        for _ in range(min(n_steps, config.num_stream_jobs)):
            ...
            if step > 0:
                ...
                rewards.append(reward)
            step += 1
            trace.loc[-1] = ["shortest queue", state, action, reward]
            trace.index = trace.index + 1
            trace = trace.sort_index()
    ...
    for ep in range(iter):
        ...
        for _ in range(min(n_steps, config.num_stream_jobs)):
            ...
            if step > 0:
                ...
                rewards.append(reward)
            step += 1
            trace.loc[-1] = ["dedicated server", state, action, reward]
            trace.index = trace.index + 1
            trace = trace.sort_index()
...
if not exists("output/universal/result.txt"):
    with open("output/universal/result.txt", 'a+') as f:
        f.write("service_rate\t")
        for alg in algs:
            f.write(alg+'\t')
        f.write('\n')
    f.close()
with open("output/universal/result.txt", 'a+') as f:
    f.write(str(config.service_rates)+'\t')
    for alg in algs:
        print(alg+'\t'+str(df[df['alg']==alg].iloc[n_steps-1]['cumulative reward']))
        f.write(str(df[df['alg']==alg].iloc[n_steps-1]['cumulative reward'])+'\t')
    f.write('\n')'''